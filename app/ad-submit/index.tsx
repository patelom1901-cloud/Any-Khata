...
export default function AdSubmitScreen() {
  ...
  const onSubmit = async (data: AdFormData) => {
    ...
    try {
      ...
      // ── Step 1: Create Cashfree payment link (same as ₹11 subscription) ──
      ...
      // ── Step 2: Open Cashfree WebView (same as ₹11 subscription) ──
      ...
      // ── Step 3: Payment succeeded — now create the ad ──
      const expiryDate = new Date();
      expiryDate.setDate(expiryDate.getDate() + 30);
      const subscription_expiry = expiryDate.toISOString().split('T')[0]; // YYYY-MM-DD
  ...
  return (
    ...
    >
      ...
      <Text style={styles.title}>{t('ad_submit.title')}</Text>
      <Text style={styles.subtitle}>
        {t('ad_submit.subtitle')}
      </Text>
